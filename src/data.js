...
const getRecentNotes = async () => {
    const response = await axios.get(`${process.env.POSTGREST_HOST}/notes?select=*&order=created_time.desc&limit=7&note_id=neq.${homeArticle}&parent_id=neq.${articlesFolder}`, config);
